...
// test is the test Number
  ...
  string temperatures_path_name = "./data/1D/sigma_" + rounded_sigma +
                                  "/simulation_" + simulation  + "/";
  string sizes_file_name =  "./data/1D/sigma_" + rounded_sigma +
                            "/simulation_" + simulation +"/Ls_test_"+test+".txt";
  string output_path_name = "./data/1D/sigma_" + rounded_sigma +
                            "/simulation_" + simulation  + "/";
  ...
  double lower = 1 ; //this is the lower bound of the distances extracted
  ...
  for(int sizes = 0; sizes < number_of_L; sizes ++){
    ...
    #pragma omp parallel
    {
      ...
      int center, direction,neighs_sum = 0, energy;
      double T,r, unif,  magne = 0;
      double filt[7];
    ...
    for (int termo = 0; termo < termalization; termo++) {
      for (int spinflip = 0; spinflip < pow(L,2); spinflip++) {
        neighs_sum = 0;
        center = (int)(L*sfmt_genrand_res53(&sfmt))%L;
        ...
        for (int j = 0; j < q; j++){
            ...
            if (r <= (int) L/2){
              ...
              direction = 2*(int)sfmt_genrand_res53(&sfmt) - 1 ;

              neighs_sum += lattice[((int)(center + r*direction) + L)%L];
            }
            

        }
        energy = lattice[center]*neighs_sum;
        unif = sfmt_genrand_res53(&sfmt);
    ...
    for (int k = 0; k < L; k++){
              ...
              fwrite(&lattice[k],sizeof(int), 1, configuration_file);
