...
def temper_label(y, N,Ntemp, beta):
...
T = np.arange(Tstart,Tend+0.1,0.1)
...
X=np.fromfile('/home/riccardo/DOTTORATO/CODE/SW/configurazioni/SW_LATT_likeli.dat' ,dtype='int32')
X=np.array(X)
...
temper_label(y_label, N,Ntemp, T)
y_label = np.array(y_label)
y_label = np.reshape(y_label,(Nconf,N))
...
train_weights, dense_biases=np.array(network.layers[-1].get_weights())
...
f2 = open('likeli_data/x_test.txt' , 'w')

for i in X_test:
    np.savetxt(f2,i)

f2.close()
f2 = open('likeli_data/y_test.txt' , 'w')
...
#3315
f1 = open("likeli_data/filter_matrix.txt", "w")
...
f = open("likeli_data/weight_matrix.txt", "w")
np.savetxt(f,train_weights)
f.close()

f = open('likeli_data/dense_biases.txt', 'w')
np.savetxt(f, dense_biases)
f.close()
f = open('likeli_data/biases.txt', 'w')
np.savetxt(f, biases)
f.close()
...
f = open('likeli_data/y_predict.txt', 'w')
